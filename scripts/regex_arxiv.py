...
__all__ = ("REGEX_ARXIV_SIMPLE", "REGEX_ARXIV_STRICT", "REGEX_ARXIV_FLEXIBLE")
...
categories = list(set(CATEGORIES + [cat.replace("-", "") for cat in CATEGORIES]))
subcategories = list(
    set(SUB_CATEGORIES + [cat.replace("-", "") for cat in SUB_CATEGORIES])
)
...
REGEX_ARXIV_STRICT = (
    r"(?:{})".format(RE_PREFIX_ARXIV) + r"(?:"
    r"({})".format(RE_ID_OLD) + r"|"
    r"({})".format(RE_ID_NEW) + r")"
)
...
REGEX_ARXIV_FLEXIBLE = (
    r"(?:"
    r"({})".format(REGEX_ARXIV_SIMPLE) + r")|(?:"  # capture
    r"(?:"
    r"(?:{})?".format(RE_PREFIX_URL) + r"(?:{})?".format(RE_PREFIX_EPRINT) + r"(?:"
    r"(?:{})?".format(RE_PREFIX_ARXIV) + r"({})".format(RE_ID_OLD) + r"|"  # capture
    ...
    r"({})".format(REGEX_ARXIV_SIMPLE) + r")|(?:"  # capture
    r"(?:[\[\(]\s*)"
    r"({})".format(REGEX_ARXIV_SIMPLE) + r"(?:\s*[\]\)])"  # capture
    r")"
)
